...
long a_circ, k_circ;		///> the two parameters of the topology.
...
struct Double map(long m){
	...
	else if(NUMNODES-a_circ <= m){
		v.x = m-NUMNODES; v.y = 0;
	}
	else{
		q = m/a_circ; r = m%a_circ;
		if(q%2 == 0){
			q = q-1; r = r+a_circ;
		}
		// At this point, m = qa +r, where q is odd and 0 <= r <= 2a-1.
		c = ((q+1)/2)*k_inv % a_circ;
		if(0 <= r && r <= 2*(a_circ-c)-1){
			v.x = r-a_circ+c; v.y = -c;
		}
		else{
			v.x = r-2*a_circ+c; v.y = a_circ-c;
		}
	}
...
long  circ_pk_neighbor(long ad, dim wd, way ww)
{
	...
	switch (wd) {
		case D_X:	//	Clockwise and counterclockwise neighbors
			if (ww == DOWN)
				res = (NUMNODES+ad-s1)%NUMNODES;
			else
				res = (ad+s1)%NUMNODES;
			break;
		case D_Y:	//	neighbors located at step2
			if (ww == DOWN)
