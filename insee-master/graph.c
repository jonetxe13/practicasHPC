...
/**
 * Creates a rrg graph topology from a file or a predefined topology (for the moment, Kautz and de Bruijn).
 *
...
void create_graph(){
	...
	graph_t *graph;	// temporary data structure to store the graph while the network topology is built. It will be freed before exiting this function
 ...
 * Each line (n) has a list with the connections of switch n, following the port order, separated by tabs.
 * Each connection is a tuple with the format (sw,pt) defining adjacency to port pt of switch sw.
 */
 ...
 * Load the description of the topology (graph) from a file.
 * The format of the file is as follows:
 * Each line contains a pair of adjacent nodes with the format n1 n2defining adjacency to port pt of switch sw.
 */
...
/**
* Builds a Generalized de Bruijn graph
*
...
/**
* Builds a Generalized Kautz graph
*
*@param a pointer to the graph
*/
...
/**
* Prints a graph in the form of an adjacency list.
* Each switch is printed in a line with its neighbors in port order.
*
*@param a pointer to the graph
*/
...
/**
* Frees data structures needed for routing within a graph.
*/
void finish_graph(){
	if(routing == CAM_ROUTING)
		finish_cams();
