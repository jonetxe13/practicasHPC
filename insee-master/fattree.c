...
void create_fattree(){
	...
	long k, ks, ks1, sg;	//Auxiliary variables for saving cpu and reducing complexity.
	...
	// Initializing processors. Only 1 transit port plus injection queues.
	for (i=0; i<nprocs; i++){
	...
	for (st=0; st< nstages-1; st++ ){
		st_first = next_st_first;						//first switch in this stage
...
/**
* Generates the routing record for a k-ary n-tree using arithmetic routing.
*
...
/**
* Generates a full routing record for a k-ary n-tree.
*
* This function only supports static routing because the route is calculated here.
*
* @param source The source node of the packet.
* @param destination The destination node of the packet.
* @return The routing record needed to go from source to destination.
...
routing_r fattree_rr (long source, long destination) {
	...
	res.rr=alloc(2*nhops*sizeof(routing_r));
	res.rr[0]=0; // first hop is always up the NIC

	for (k=1; k<nhops; k++){
		res.rr[k]=rand() % stUp;
	}
	for (k=nhops; k<2*nhops; k++){
        res.rr[k]=((destination / (long)pow(stDown, (2*nhops)-k-1)) % stDown ) + stUp;
	}
...
/**
* Generates a full routing record for a thin-tree (random non-deterministic).
*
* This function only supports static routing because the route is calculated here.
*
* @param source The source node of the packet.
...
/**
* Generates a full routing record for a thin-tree (deterministic based on both the source and destination).
*
* This function only supports static routing because the route is calculated here.
*
* @param source The source node of the packet.
...
routing_r thintree_rr (long source, long destination) {
	...
	for (k=1; k<nhops; k++){
		res.rr[k]=((destination+source)/ (long)pow(stDown, k-1)) % stUp;
	}
	for (k=nhops; k<2*nhops; k++){
        res.rr[k]=((destination / (long)pow(stDown, (2*nhops)-k-1)) % stDown ) + stUp;
	}
...
/**
* Generates a full routing record for a thin-tree (deterministic based on the source id).
*
* This function only supports static routing because the route is calculated here.
*
* @param source The source node of the packet.
...
routing_r thintree_rr_src (long source, long destination) {
	...
	for (k=1; k<nhops; k++){
		res.rr[k]=(source/ (long)pow(stDown, k-1)) % stUp;
	}
	for (k=nhops; k<2*nhops; k++){
        res.rr[k]=((destination / (long)pow(stDown, (2*nhops)-k-1)) % stDown ) + stUp;
	}
...
/**
* Generates a full routing record for a thin-tree (deterministic based on the destination id).
*
* This function only supports static routing because the route is calculated here.
*
* @param source The source node of the packet.
...
routing_r thintree_rr_dst (long source, long destination) {
	...
	for (k=1; k<nhops; k++){
		res.rr[k]=(destination / (long)pow(stDown, k-1)) % stUp;
	}
	for (k=nhops; k<2*nhops; k++){
        res.rr[k]=((destination / (long)pow(stDown, (2*nhops)-k-1)) % stDown ) + stUp;
	}
...
/**
* Generates a full routing record for a slim-tree.
*
* This function only supports static routing because the route is calculated here.
*
* @param source The source node of the packet.
