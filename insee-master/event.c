...
* When it reaches the length of the event, this is erased from the queue.
*
* @param q A pointer to an event queue.
* @param i A pointer to the event to do.
* @param increment the number of times the event has happened.
*/
void do_event_n_times (event_q *q, event *i, CLOCK_TYPE increment) {
	...
	if (e->ev.type!=i->type ||
		e->ev.pid!=i->pid ||
		e->ev.task!=i->task) {
			panic("Incorrect event in do_event_n_times");
		}

	e->ev.count+=increment;
	if (e->ev.count == e->ev.length){
		q->head=q->head->next;
		free (e);
		if (q->head==NULL)
			q->tail=NULL;
	} else 	if (e->ev.count > e->ev.length){
...
#if (TRACE_SUPPORT > 1)
/**
* Initializes all occurred events lists.
*
* There is a list in each router for each possible source of messages.
...
void ins_occur (event_l **l, event i){
	...
	if (e->ev.type == i.type && e->ev.pid == i.pid &&
		...
		// The occurrence is the first element.
		e->ev.count++;
		return;
	}
...
* @return TRUE if the event has been occurred, or FALSE if it hasn't
*/
...
#endif // Trace support with multi/single list #occurs
