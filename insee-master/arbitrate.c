...
void arbitrate_init(void) {
	...
	else // Trees and hybrid topologies.
	    last_port_arb_con=(nchan * nnics);
}

/**
* Frees the data structures used for arbitration
*/
...
* @param d_p The destination port for which the arbitration is performed.
*
* @see arbitrate
* @see arbitrate_select
* @see arbitrate_select_fifo
* @see arbitrate_select_longest
* @see arbitrate_select_highest
...
void arbitrate_direct(long i, port_type d_p) {
	...
	if (ipr_l[network[i].congested] && (rand() <= ipr_l[network[i].congested]))
		lastlimit = p_inj_first;	// If priority is ON for in_transit traffic, injection ports are not included in the arbitration process
	else
        lastlimit = p_con;
...
void arbitrate_icube(long i, port_type d_p) {
	...
	if ((s_p == NULL_PORT) && (firstlimit != 0) && (i>=nprocs))
		s_p = arbitrate_select(i, d_p, 0, stDown*nchan);
	if (s_p != NULL_PORT){
		reserve(i, d_p, s_p);
	}
}
...
/**
* Main arbitration function for trees, dragonfly and other arbitrary functions. Implements the virtual function arbitrate.
*
* This functions will only check those ports that make sense:
* the NIC when arbitrating a consumption port,
* the injection ports when arbitrating a NIC,
...
void arbitrate_arbitrary (long i, port_type d_p) {
	...
	if (i<nprocs){ // In a leaf node.
		if (d_p==p_con){	// only the NIC(s) will require using the consumption port.
            firstlimit = 0;
			lastlimit = nchan * nnics;
		...
		lastlimit = p_inj_first;
		if (ipr_l[network[i].congested] && (rand() <= ipr_l[network[i].congested]))
			firstlimit = stDown * nchan;	// If priority is ON for in_transit traffic, ports connected to servers are not included in the arbitration process
		else
			firstlimit = 0;
	}

	s_p = arbitrate_select(i, d_p, firstlimit, lastlimit);
	if ((s_p == NULL_PORT) && (firstlimit != 0 && firstlimit != p_inj_first)) // arbitrate the remaining (low-priority) ports
		s_p = arbitrate_select(i, d_p, 0, (stDown * nchan));
	if (s_p != NULL_PORT)
		reserve(i, d_p, s_p);
}
...
* @return The selected port, or NULL_PORT if none is selected.
*
* @see arbitrate
* @see arbitrate_select
*/
...
/**
* Select the port with the highest queue length.
*
...
/**
* Select the port with the highest VC.
*
* Given a range of input-injection ports, select the one whose VC is higher.
* We take a look to the queues in a round-robin fashion, starting with the last lucky
* input port (network[i].p[d_p].ri), if several ports have the same VC, the first
* visited will be the selected one.
*
* @param i The node in which the arbitration is performed.
* @param d_p The output port for which the arbitration is performed.
* @param first The first port of the range we need to arbitrate.
* @param last The last port of the range. This port is not included.
* @return The selected port, or NULL_PORT if none is selected.
*
* @see arbitrate
* @see arbitrate_select
...
port_type arbitrate_select_highest(long i, port_type d_p, port_type first, port_type last) {
	port_type s_p, selected_port=NULL_PORT, visited;
	long cv_of_selected, pl;
	long dif=last-first;

	s_p = ((network[i].p[d_p].ri + 1) % dif);
	if (s_p >= last || s_p<first)
		s_p = first;
	cv_of_selected = -1;
	...
	return(selected_port); // NULL_PORT if no requesters
}
