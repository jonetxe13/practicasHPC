/**
 @file
* @brief	Implementation of a double linked dynamic list.
...
listElement *CreateListElement( void * dataPointer ){
	...
	firstListElement = (listElement *) alloc( sizeof(listElement) * 1 );
...
listElement *AddBeforeElement( listElement *thisElement, void *dataPointer ){
	...
	if( thisElement->pointer) {
		newListElement = (listElement *) alloc( sizeof(listElement) * 1 );
...
list *CreateVoidList(){
	...
	newList = (list *) alloc( sizeof( list ) * 1 );
	newList->emptyList = B_TRUE;
	newList->first = NULL;
	newList->last = NULL;
...
* @param theList The lñist to destroy.
*/
void  DestroyListData( list **theList ){
	listElement *ll1;
	listElement *ll2;
	list *thisList = *theList;
	if( ! thisList ) return;
...
int   IsInList( list *thisList, void *dataPointer ){
	...
	if( !thisList->emptyList ) {
		ll = thisList->first;
		while( ll ) {
			if( dataPointer == ll->pointer) return B_TRUE;
			ll = ll->next;
		}
	}
	return B_FALSE;
...
void PrintList( list *thisList ){
	...
	if( ! thisList )
		...
		ll = thisList->first;
		while( ll ) {
			printf(" %ld %ld %ld \n",*(long *)ll->previous,*(long *)ll->pointer,*(long *)ll->next );
			ll = ll->next;
		}
	}
	printf("===============\n");
