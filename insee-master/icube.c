...
* of them. Origins are in nodes (0,0) (X/2, 0) (0, Y/2) and (X/2, Y/2) respectively
...
routing_r icube_4mesh_rr (long source, long destination) {
	...
	p= (source % stDown) % links_per_direction;
	...
	if (ndim>2){
		panic("4mesh only defined for 2D indirect cubes");
	}
...
routing_r icube_1mesh_rr (long source, long destination) {
	...
	res.rr[ndim] = (source % stDown) % links_per_direction; // Last dimension. Stores the number of parallel mesh.
	return res;
}
		...
		for (j=0; j<ndim; j++) { // X, Y, Z
			for (k=0; k<nways; k++){ // +, -
				...
				for (n=0; n<links_per_direction; n++){ // 0 1 2
					p  = stDown + (links_per_direction*((2*j)+k)) +n;
					np = stDown + (links_per_direction*((2*j)+1-k)) +n;
