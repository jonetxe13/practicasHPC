...
void print_partials(void) {
	...
	if(pheaders & 2){
		iload = (double) (sent_phit_count) / (1.0 * nprocs * copyclock);
		printf(", %7.5lf", iload);
	}
	if(pheaders & 4){
		cload = (double) (rcvd_phit_count) / (1.0 * nprocs * copyclock);
		printf(", %7.5lf", cload);
	}
	if(pheaders & 8)
...
/* INPUT SECTION */
	...
	if (topo < DIRECT){
		...
		printf(" p:%ld, a:%ld, h:%ld - random connections\n",param_p,param_a,param_h);
		printf("endpoints, switches(radix):       %-10ld %ld (%ld)\n", nprocs, NUMNODES - nprocs, radix);
	}
