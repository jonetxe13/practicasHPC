...
long used_chan = 0;	///< Number of used Virtual Channels (default 0).
long diameter_t = 0;          ///< Used for graph topologies
long diameter_r = 0;          ///< Used for graph topologies
...
long nswitches;         ///< Total number of swicthes

char topology_filename[128];
...
/**
* Id of the policy to fill out the CAM tables.
*
* @see cam_policy_t
* @see cam_policy_l
*/
cam_policy_t cam_policy;
vc_inj_t vc_inj;
cam_ports_t cam_ports;
long cam_policy_params[3];
...
long shift_plc;				///< Number of places for shift placement.
...
CLOCK_TYPE warm_up_period,	///< Number of 'oblivious' cycles until start convergency asurement simulation.
	warmed_up;			///< The cycle in which warming are really finished.
...
dim * port_coord_dim;			///< An array containing the dimension for each port.
way * port_coord_way;			///< An array containing the direction (way) for each port.
channel * port_coord_channel;	///< An array containing the number of virtual channel for each port.
...
/**
* 'Virtual' Function that calculates next port using the rr or the cam tables.
* @param source The current packet
* @return The port to be used for the next hop.
*
* @see init_functions
* @see get_next_hop_rr
* @see get_next_hop_cam
*/
long (*get_next_hop) (packet_t * pkt);
long (*get_cam_port) (packet_t * pkt);

/**
* 'Virtual' Function that calculates the port that will be used in the next router
* to be able to perform VOQ; using the rr or the cam tables.
* @param source The current packet
* @return The port to be used for the next hop.
*
* @see init_functions
* @see get_next_router_hop_rr
* @see get_next_router_hop_cam
*/
long (*get_next_router_hop) (packet_t * pkt);

void (*init_cams)();
void (*finish_cams)();
void (*fill_cam)(long switch_id, graph_t *graph);
void (*set_mpaths)(path_t **path, long n_paths);
void (*set_mpaths_reverse)(path_t **path, long n_paths);
...
* @see check_rr_fattree_arithmetic
* @see check_rr_fattree_adaptive
* @see check_rr_slimtree_arithmetic
* @see check_rr_slimtree_adaptive
* @see check_rr_thintree_arithmetic
