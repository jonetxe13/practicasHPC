...
/**
* Structure in which all the packets are stored.
*
...
*
* Normally is the number of nodes multiplied by the maximum capacity of each router.
*/
long pkt_max;
...
void pkt_init(){
	...
	if(topo<DIRECT){
		...
		pkt_max = (((NUMNODES-nprocs) * n_ports * buffer_cap)	// packets in network +
				    + (nprocs * ((ninj * binj_cap)				// packets in injectors .
								  +(nnics * nchan * buffer_cap))));	// + packets in nics.
	}
