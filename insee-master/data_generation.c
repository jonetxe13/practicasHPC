...
long pop[POP_SIZE];	///< The population when using population based distributions. Should be implemented dynamically only when needed.
...
*
* When using DIMENSION_ORDER_ROUTING most packets are injected in X+ or X-
*
* @param i The node in which the injection is performed.
		...
		case POPULATION:
			if (topo<DIRECT){	// for direct topologies, ttorus should have a separate one.
				long dst[3]={0,0,0},	// the number of hops in each dimension.
				     r;			// the target number of hops. may not be reached if r is too high
				do{
					r=pop[rand()%POP_SIZE];
				...
				if (ndim==3){
					dst[D_Z]=rand()%(1+r);
					if (dst[D_Z]>nodes_z/2) // limit the number of hops in Z to nodes_z/2
						dst[D_Z]=nodes_z/2;
					r-=dst[D_Z]; // update the remaining hops

					if (rand() & 1) // choose the direction randomly
						dst[D_Z]=-dst[D_Z];
				}
				if (ndim>1){
					dst[D_Y]=rand()%(1+r);
					if (dst[D_Y]>nodes_y/2) // limit the number of hops in Y to nodes_y/2
						dst[D_Y]=nodes_y/2;
					r-=dst[D_Y]; // update the remaining hops
				...
				dst[D_X]=r;
				if (dst[D_X]>nodes_x/2) // limit the number of hops in X to nodes_x/2
					dst[D_X]=nodes_x/2;
...
*
* Performs data movement from an injection buffer, which is used for data transmission between
	...
	for (i=0; i<nprocs; i++) {
		...
		switch (pattern) {
			...
			case ADV:
				...
				if (topo!=DRAGONFLY_ABSOLUTE && topo!=DRAGONFLY_RND && topo!=DRAGONFLY_CIRCULANT && topo!=DRAGONFLY_RELATIVE && topo!=DRAGONFLY_NAUTILUS && topo!=DRAGONFLY_HELIX && topo!=DRAGONFLY_OTHER)
